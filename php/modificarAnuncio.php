  ...
  <form class= "grid-container" action="modificarAnuncio.php" method="post" enctype="multipart/form-data">
    ...
    <?php
        if (isset($_POST["submit"]) && $_FILES["image"]['size']!=0) {
            ...
            $insert = $conn->query("UPDATE anuncio set titulo = '$titulo', descripcion ='$descripcion', idsubcategoria = '$subcategoria', idubicacion = '$ubicacion', telefono = '$telefono', datostecnicos ='$datostecnicos', masinformacion = '$masinformacion', precio = '$precio' where idanuncio = $idanuncio");
            if ($insert){
                echo "nice";
            } else {
